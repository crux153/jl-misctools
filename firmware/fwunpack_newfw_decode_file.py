...
def parse_newfw_file(fw, outdir:Path):
    for i, ent in enumerate(JLFSIterator(fw, 0, 0)):
        if i == 0:
            print('(App Area Head)', ent)
        else:
            print('(Res)', ent)
            # print('(Res)', ent.offset)
            # print('(Res)', ent.size)
            # print('(Res)', ent.flags)

            fpath = outdir / ent.name
            fpath.write_bytes(fw[ent.data_offset : ent.data_offset + ent.data_size])
...
def parsefwfile(path:Path, outdir:Path):
    # Load firmware data
...
for path in args.input:
    print(f'#\n# {path}\n#\n')
    try:
        parsefwfile(path, Path(args.dirname.format(fpath=path, fname=path.name, fdir=path.parent)))
